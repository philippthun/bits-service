...
var _ = Describe("Registry", func() {
	...
	BeforeSuite(func() {
		...
		dropletBlobstore = inmemory_blobstore.NewBlobstoreWithEntries(map[string][]byte{"the-droplet-guid": droplet})
	...
	Describe("pull image", func() {
		It("should serve the GET image manifest endpoint", func() {
			res, e := http.Get(serverURL + "/v2/the-image/manifests/the-droplet-guid")
			...
			res, e = http.Get(serverURL + "/v2/the-image/blobs/e45cc3b91fb3e12c17e8a9a3a9d476f516f480756795a2b446f0a129f6bcae3d")
			Expect(e).NotTo(HaveOccurred())
			Expect(ioutil.ReadAll(res.Body)).To(MatchJSON(`{
				"config": {
				  "user": "vcap"
			...
			res, e = http.Get(serverURL + "/v2/the-image/blobs/56ca430559f451494a0e97ff4989ebe28b5d61041f1d7cf8f244acc76974df20")
			Expect(e).NotTo(HaveOccurred())
			Expect(ioutil.ReadAll(res.Body)).To(Equal([]byte("the-rootfs-blob")))

			res, e = http.Get(serverURL + "/v2/the-image/blobs/ccba5ce536c29da80ff2da1c81fc7b9e4d07ab679b6bfb03964432f116d61dd7")
			Expect(e).NotTo(HaveOccurred())
		...
		Context("image names have multiple paths or special chars", func() {
			It("supports / in the name path parameter", func() {
				res, e := http.Get(serverURL + "/v2/image/name/manifests/the-droplet-guid")

				Expect(res.StatusCode, e).To(Equal(http.StatusOK))
			})

			It("supports multiple / in the name path parameter", func() {
